...
out_other_families_csv=$out_dir/"$project_name"_analyze_other_families.csv
out_oaf_csv=$out_dir/"$project_name"_analyze_oaf.csv
out_file=$out_dir/"$project_name"_analyze.xls

echo "## analyze result from family 242 exome" 1>&2
echo "## parameters" 1>&2
...
echo -e "$parsed_header" > $out_other_families_csv
...
#            parsed_rec+="\t"`cut_gt "$rec" "$j"`
    ...
    echo -e "$parsed_rec" >> $out_other_families_csv
#    for (( j=1; j<=$n_col; j++ ))
#    do
#        if [ $j -ne 3 ] && [ $j -ne 6 ] && [ $j -ne 7 ] && [ $j -ne 8 ] && [ $j -ne 9 ]; then
...
csv2xls_cmd="python $csvs2xls_highlight_MT $out_file $out_other_families_csv \"other families\" $out_oaf_csv \"oaf\""
eval $csv2xls_cmd
